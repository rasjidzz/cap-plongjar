...
namespace Tests\Feature\KaurLab; // Perhatikan namespace ini
...
class ViewPlotingPengajaranTest extends TestCase
{
    ...
    /**
     * Test Case: Memastikan Kaur Lab dapat melihat hasil plottingan pengajaran berdasarkan prodi dan tahun ajaran.
    ...
    public function test_laa_can_view_plotting_results_by_prodi_and_tahun_ajaran_successfully(): void
    {
        ...
        $programStudiTarget = ProgramStudi::factory()->create(['nama' => 'S1 Teknik Komputer']);
        $programStudiOther = ProgramStudi::factory()->create(['nama' => 'S1 Fisika']);

        // Buat Dosen, Mata Kuliah, Mapping, dan Plottingan untuk prodi dan tahun ajaran TARGET
        ...
        // Perbaikan: Buat KoordinatorMatakuliah dengan id_mapping_kelas_matakuliah yang valid langsung
        KoordinatorMatakuliah::factory()->create([
            'id_dosen' => $dosenKoordinator->id,
            'id_mapping_kelas_matakuliah' => $mapping1->id, // Langsung kaitkan ke mapping yang valid
        ]);
        ...
        PlottinganPengajaran::create([
            ...
            'beban_sks' => 3, // SKS penuh untuk solo
        ]);
        PlottinganPengajaran::create([ // Tambahkan plottingan kedua untuk prodi target
            'id_dosen' => $dosen2->id,
            'id_mapping_kelas_matakuliah' => $mapping2->id,
            'beban_sks' => 1, // Contoh beban SKS untuk team teaching
        ]);


        // Buat plottingan untuk Program Studi LAIN (agar tidak ikut terambil)
        $matakuliahLain = Matakuliah::factory()->create(['id_pic' => Pic::first()->id, 'sks' => 4]);
        ...
        // 2. Aksi: Login sebagai Kaur Lab dan kirim GET request
        // Endpoint: /api/v1/plottingan-pengajaran/tahun-ajaran/{id_tahun_ajaran}/program-studi/{id_program_studi}
        $response = $this->actingAs($kaurLabUser, 'sanctum')->getJson(
        ...
        $response->assertJsonFragment([
            'id_plottingan' => PlottinganPengajaran::where('id_dosen', $dosen1->id)->where('id_mapping_kelas_matakuliah', $mapping1->id)->first()->id,
            'nama_matakuliah' => $matakuliah1->nama_matakuliah,
            'kode_matakuliah' => $matakuliah1->kode_matkul,
            'pic_matakuliah' => Pic::first()->name,
            ...
            'beban_sks_dosen' => 3, // Perbaikan: nilai aktual
        ...
        // Assert for the second plotting item
        $response->assertJsonFragment([
            'id_plottingan' => PlottinganPengajaran::where('id_dosen', $dosen2->id)->where('id_mapping_kelas_matakuliah', $mapping2->id)->first()->id,
            'nama_matakuliah' => $matakuliah2->nama_matakuliah,
            'kode_matakuliah' => $matakuliah2->kode_matkul,
            'pic_matakuliah' => Pic::first()->name,
            'sks_matakuliah' => $matakuliah2->sks,
            'nama_kelas' => $mapping2->nama_kelas,
            'dosen_pengajar' => $dosen2->name,
            'kode_dosen_pengajar' => $dosen2->lecturer_code,
            'beban_sks_dosen' => 1, // Perbaikan: nilai aktual
            // koordinator_matakuliah dan kode_koordinator mungkin null jika tidak di-assign ke mapping2
            'tahun_ajaran' => $tahunAjaran->tahun_ajaran . ' - ' . $tahunAjaran->semester,
            'mandatory_status' => $matakuliah2->mandatory_status,
            'tingkat_matakuliah' => $matakuliah2->tingkat_matakuliah,
            'hour_target' => $matakuliah2->hour_target,
            'mk_eksepsi' => $matakuliah2->matakuliah_eksepsi,
            'team_teaching' => ($mapping2->team_teaching ? 'Yes' : 'No'),
        ]);
    ...
    /**
     * Test Case: Memastikan Kaur Lab mendapatkan daftar plottingan kosong jika tidak ada plottingan untuk prodi dan tahun ajaran tersebut.
     * HTTP Response status code = 200 (Success)
     *
     * @return void
     */
    public function test_kaur_lab_gets_empty_plotting_results_if_no_records_for_prodi_and_tahun_ajaran(): void
    {
        // 1. Persiapan Data
        $kaurLabUser = $this->createKaurLabUser();
        $tahunAjaran = TahunAjaran::factory()->create(['tahun_ajaran' => '2023/2024', 'semester' => 'ganjil']);
        $programStudi = ProgramStudi::factory()->create(['nama' => 'S1 Kimia']);
        // Tidak membuat plottingan apapun untuk prodi dan tahun ajaran ini
    ...
    /**
     * Test Case: Memastikan Kaur Lab gagal melihat hasil plottingan jika Tahun Ajaran tidak ditemukan.
     * HTTP Response status code = 404 (Not Found)
     *
     * @return void
     */
    public function test_kaur_lab_cannot_view_plotting_results_if_tahun_ajaran_not_found(): void
    {
        // 1. Persiapan Data
        $kaurLabUser = $this->createKaurLabUser();
    ...
    /**
     * Test Case: Memastikan Kaur Lab gagal melihat hasil plottingan jika Program Studi tidak ditemukan.
     * HTTP Response status code = 404 (Not Found)
     *
     * @return void
     */
    public function test_kaur_lab_cannot_view_plotting_results_if_program_studi_not_found(): void
    {
        // 1. Persiapan Data
        $kaurLabUser = $this->createKaurLabUser();
